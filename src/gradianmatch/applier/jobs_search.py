...
def build_query(cv_text: str, conditions: str, claude) -> QuerySpec:
    if claude is not None and (cv_text or conditions):
        data = claude.run_json(_QUERY_PROMPT + (conditions or cv_text))
        return QuerySpec(query=data.get("query", ""), location=data.get("location", ""))
    words = re.findall(r"[a-zA-Zà-úÀ-Ú]{3,}", (conditions or cv_text).lower())
...
def find_jobs(cv_text, conditions, platform_ids, claude, http, cfg,
    ...
    terms = {w for w in re.findall(r"[a-z]{3,}", spec.query.lower())}
