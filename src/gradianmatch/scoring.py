...
def _ats(cv: Resume, offer: OfferReqs, tax: SkillTaxonomy):
    ...
    for s in must:
        (matched if present(s) else missing).append(s)
    nice_hits = [s for s in nice if present(s)]
    must_score = (len([s for s in must if present(s)]) / len(must)) if must else 1.0
    nice_score = (len(nice_hits) / len(nice)) if nice else 1.0
    score = round(100 * (0.8 * must_score + 0.2 * nice_score))
    detail = f"{len(matched)}/{len(must)} must-have skills present"
    return CategoryScore("ATS keyword coverage", score, WEIGHTS["ats"], detail), matched, missing

def _cv_years(cv: Resume) -> int:
    years = 0
    for w in cv.work:
        try:
            start = int((w.startDate or "0")[:4]); end = int((w.endDate or "2026")[:4])
            years += max(0, end - start)
        except ValueError:
            continue
    return years

def _gating(cv: Resume, offer: OfferReqs):
    checks, passed = 0, 0
    notes = []
    if offer.languages:
        checks += 1
        cv_langs = {l.language.lower() for l in cv.languages}
        if all(any(req.lower() in c for c in cv_langs) for req in offer.languages):
            passed += 1
        else:
            notes.append(f"Missing language(s): {', '.join(offer.languages)}")
    if offer.min_years:
        checks += 1
...
def score_compatibility(cv: Resume, offer: OfferReqs, semantic: SemanticFit,
    ...
    ats, matched, missing = _ats(cv, offer, taxonomy)
    gate = _gating(cv, offer)
    sem = CategoryScore("Semantic fit", int(semantic.score_0_100), WEIGHTS["semantic"], semantic.rationale)
    overall = round(ats.score_0_100 * WEIGHTS["ats"] + gate.score_0_100 * WEIGHTS["gating"]
                    + sem.score_0_100 * WEIGHTS["semantic"])
    gaps = [f"Missing must-have: {m}" for m in missing] + list(semantic.red_flags)
